// app/dashboard/job-seeker/applications/ApplicationTable.jsx
"use client";
...
const getStatusChip = (status = "Applied") => {
  ...
  switch (normalized) {
    case 'applied':
      return <Chip variant="bordered" className="border-blue-500/50 text-blue-400 text-xs font-medium px-3 py-1 bg-blue-500/10">Applied</Chip>;
    case 'review':
    case 'reviewing':
      return <Chip variant="bordered" className="border-yellow-500/50 text-yellow-400 text-xs font-medium px-3 py-1 bg-yellow-500/10">Review</Chip>;
    case 'shortlisted':
      return <Chip variant="bordered" className="border-green-500/50 text-green-400 text-xs font-medium px-3 py-1 bg-green-500/10">Shortlisted</Chip>;
    case 'rejected':
      return <Chip variant="bordered" className="border-red-500/50 text-red-400 text-xs font-medium px-3 py-1 bg-red-500/10">Rejected</Chip>;
    case 'offered':
      return <Chip variant="bordered" className="border-purple-500/50 text-purple-400 text-xs font-medium px-3 py-1 bg-purple-500/10">Offered</Chip>;
    case 'interview':
      return <Chip variant="bordered" className="border-indigo-500/50 text-indigo-400 text-xs font-medium px-3 py-1 bg-indigo-500/10">Interview</Chip>;
    default:
      return <Chip variant="bordered" className="border-gray-500/50 text-gray-400 text-xs font-medium px-3 py-1 bg-gray-500/10">{status}</Chip>;
  }
};
  ...
  if (!jobs || jobs.length === 0) {
    return (
      <div className="bg-gray-900/50 border border-white/10 rounded-2xl p-12 text-center">
        ...
        <p className="text-gray-400 mb-6">Start applying to jobs and track your progress here</p>
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-white/5">
            {jobs.map((job, index) => {
              ...
              return (
                <tr key={job._id?.$oid || job.jobId || index} className="hover:bg-white/5 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">{workType} • {locationType}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <span className="text-sm text-gray-400">
                      {formatRelativeTime(job.createdAt?.$date || job.createdAt || job.appliedDate)}
                    </span>
                  </td>
                  <td className="px-6 py-4">
